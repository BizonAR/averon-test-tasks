...
void TRichEditFormatter::ToggleBold()
{
	...
	ApplyFormatting(!(charFormat.dwEffects & CFE_BOLD), charFormat.dwEffects
		...
		static_cast<TColor>(charFormat.crTextColor));
}
...
void TRichEditFormatter::SetFontColor(TColor Color)
{
    TCharFormat charFormat = GetCharFormat();
	ApplyFormatting(charFormat.dwEffects & CFE_BOLD,
					charFormat.dwEffects & CFE_ITALIC,
					charFormat.dwEffects & CFE_UNDERLINE,
					charFormat.dwEffects & CFE_STRIKEOUT, Color);
}
